...
fn certain_side_priority(cols: usize, side_size: usize, center_size: usize, padding: usize) -> usize {
    let right_side_div;
    if cols % 2 == 0 {
        if (cols / 2) - side_size < (center_size / 2) {
            0
        } else {
            right_side_div = center_size / 2;
            (cols / 2) - side_size - right_side_div
        }
    } else {
        let calc = (cols / 2) - side_size;
        if calc > padding {
            0
        } else {
            calc
...
mod tests {
    ...
    fn imported_python_test_cases() {
        ...
        assert_eq!(find_padding(7, 2, 2, 2), (0, 1));
    }
}
